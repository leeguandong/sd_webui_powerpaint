...
from pipeline.utils import TokenizerWrapper, add_tokens
from modules import script_callbacks

# 设置模型保存目录路径
models_dir = "/root/stable-diffusion-webui/extensions/sd_webui_powerpaint/models/"

if not os.path.exists(models_dir) or len(os.listdir(models_dir)) == 0:
    # 构造下载命令
    command = "python extensions/sd_webui_powerpaint/hf_download.py --model JunhaoZhuang/PowerPaint-v1 --save_dir " + models_dir
    # 执行下载命令
    os.system(command)
else:
    print("Models directory is not empty. Skipping download.")
...
pipe = Pipeline.from_pretrained('runwayml/stable-diffusion-inpainting', torch_dtype=weight_dtype,
                                cache_dir="/root/autodl-tmp/models/powerpaint")
pipe.tokenizer = TokenizerWrapper(from_pretrained='runwayml/stable-diffusion-v1-5', subfolder='tokenizer',
                                  revision=None, cache_dir="/root/autodl-tmp/models/powerpaint")
...
load_model(pipe.unet, "/root/autodl-tmp/models/powerpaint/models/models--JunhaoZhuang--PowerPaint-v1/unet/unet.safetensors")
load_model(pipe.text_encoder, "/root/autodl-tmp/models/powerpaint/models/models--JunhaoZhuang--PowerPaint-v1/text_encoder/text_encoder.safetensors")
pipe = pipe.to('cuda')

depth_estimator = DPTForDepthEstimation.from_pretrained('Intel/dpt-hybrid-midas',
                                                        cache_dir="/root/autodl-tmp/models/powerpaint").to('cuda')
feature_extractor = DPTFeatureExtractor.from_pretrained('Intel/dpt-hybrid-midas',
                                                        cache_dir="/root/autodl-tmp/models/powerpaint")
openpose = OpenposeDetector.from_pretrained('lllyasviel/ControlNet', cache_dir="/root/autodl-tmp/models/powerpaint")
hed = HEDdetector.from_pretrained('lllyasviel/Annotators', cache_dir="/root/autodl-tmp/models/powerpaint")
...
def predict(input_image, prompt, fitting_degree, ddim_steps, scale, seed,
    ...
    result_m = Image.fromarray((result_m.astype('float') * (1 - mask_np.astype('float') / 512.0) +
...
def infer(input_image, text_guided_prompt, text_guided_negative_prompt,
          ...
          controlnet_conditioning_scale, removal_prompt, removal_negative_prompt):
    if task == "text-guided":
...
def on_ui_tabs():
    with gr.Blocks(analytics_enabled=False) as powerpaint:
        with gr.Row():
            with gr.Column():
                ...
                with gr.Tab('Text-guided object inpainting') as tab_text_guided:
                    ...
                    enable_control = gr.Checkbox(label='Enable controlnet',
                ...
                with gr.Tab('Object removal inpainting') as tab_object_removal:
                    enable_object_removal = gr.Checkbox(
                        label='Enable object removal inpainting', value=True,
                ...
                with gr.Tab('Image outpainting') as tab_image_outpainting:
                    enable_object_removal = gr.Checkbox(
                        label='Enable image outpainting', value=True,
                        info='The recommended configuration for the Guidance Scale is 10 or higher. \
                    ...
                    horizontal_expansion_ratio = gr.Slider(label='horizontal expansion ratio',
                                                           minimum=1, maximum=4, step=0.05, value=1, )
                    vertical_expansion_ratio = gr.Slider(label='vertical expansion ratio',
                                                         minimum=1, maximum=4, step=0.05, value=1, )
                ...
                with gr.Tab('Shape-guided object inpainting') as tab_shape_guided:
                    enable_shape_guided = gr.Checkbox(label='Enable shape-guided object inpainting',
                    ...
                    fitting_degree = gr.Slider(label='fitting degree',
                ...
                with gr.Accordion('Advanced options', open=False):
                    ...
                    scale = gr.Slider(
                        ...
                        minimum=0.1, maximum=30.0, value=7.5, step=0.1)
            ...
            run_button.click(
                ...
                inputs=[
                    ...
                    outpaint_negative_prompt, controlnet_conditioning_scale, removal_prompt, removal_negative_prompt
                ],
                outputs=[inpaint_result, gallery])

    return [(powerpaint, "PowerPaint", 'powerpaint')]
